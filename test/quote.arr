(
   ...
   test->("quote unary",         expect->(stringify->(#-1.234567),
                                          "-1.234567"))
   test->("quote if",            expect->(stringify->(#if true t, elseif a b, else f),
                                          "if true t elseif a b else f"))
   test->("quote for",           expect->(stringify->(#for x in [a b c] x),
                                          "for x in [a b c] x"))
   test->("quote loop",          expect->(stringify->(#loop until true foo),
                                          "loop until true foo"))
   test->("quote case",          expect->(stringify->(#case a of (b=>2, default=>3)),
                                          "case a of (b => 2 default => 3)"))
